...
async function resolveOrCreateCustomer(
  ...
  if (options.userId) {
    try {
      ...
      console.warn("customers.search unavailable, falling back to email:", (err as Error).message);
    }
  }
  if (options.email) {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { priceId, customerEmail, userId, returnUrl, environment } = body as {
      priceId: string; customerEmail?: string; userId?: string; returnUrl: string; environment: StripeEnv;
    };
